...
WIFI_MIMO_TEST_API int WIFI_MIMO_technologyRegister()
{
    /*---------------------*
     * Create the timer ID *
     *---------------------*/
    ...
    {
        ::TIMER_CreateTimer("WiFi_MiMo_Test", &g_WiFi_Test_timer);
    }
    else
    {
    ...
    {
        ::LOGGER_CreateLogger("WiFi_MiMo_Test", &g_WiFi_Test_logger);
    }
    else
    {
    ...
    TM_RegisterTechnology("WIFI_MIMO", &technologyID);
    if( technologyID>=0 )
    {  
		...
		if( -1==g_Logger_ID )
		{
			::LOGGER_CreateLogger("WiFi_MiMo_Test_Log", &g_Logger_ID);
