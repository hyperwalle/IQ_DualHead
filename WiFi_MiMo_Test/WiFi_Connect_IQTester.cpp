...
WIFI_MIMO_TEST_API int WiFi_Connect_IQTester(void)
{
	...
	try
	{
		...
		*---------------------------------------*/
        ...
        if( (0==g_Tester_Number) || g_Tester_Reconnect )
        {
			...
			//IQlite Merge; Tracy Yu; 2012-04-05
            ...
            {
                err = -1;
                LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] Failto get IQTester version information.\n");
				throw logMessage;
            }
            else
            {
    ...
    {
		g_WiFi_Test_ID = -1;
		ReturnErrorMessage(l_connectTesterReturn.ERROR_MESSAGE, "[WiFi] Unknown Error!\n");
		err = -1;
    }
