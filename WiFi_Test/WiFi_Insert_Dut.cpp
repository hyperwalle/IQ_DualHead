...
WIFI_TEST_API int WiFi_Insert_Dut(void)
{
	...
	char tempBuff[MAX_BUFFER_SIZE] = {'\0'}; // -cfy@sunnyvale, 2012/3/13-
	...
	try
	{
		...
		*---------------------------------------*/
        ...
        vDUT_ClearParameters(g_WiFi_Dut);	// -cfy@sunnyvale, 2012/3/13-
		...
		if( 0!=strlen(l_insertDutParam.DUT_DLL_FILENAME) ) 
		{
			...
			for (size_t i=0; i<=strlen(l_insertDutParam.DUT_DLL_FILENAME) ; i++)
				tempBuff[i] = toupper(l_insertDutParam.DUT_DLL_FILENAME[i]);
			...
			{
				err = ::vDUT_RegisterDutDll( "WIFI", l_insertDutParam.DUT_DLL_FILENAME, &g_WiFi_Dut );
				if ( ERR_OK!=err )
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] Fail to load Dut Dll.\n");
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] vDUT_RegisterDutDll() load Dut Dll return OK.\n");
				}				
			}
			else
			{
		...
		{			
			...
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] %s is not a valid DUT DLL name.\n", l_insertDutParam.DUT_DLL_FILENAME);	
			throw logMessage;
		}
		...
		if( (0==err)&&(g_WiFi_Dut>=0) )  // Status is OK
		{
			...
			{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
				...
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] %s fail to run INSERT_DUT.\n", l_insertDutParam.DUT_DLL_FILENAME);
					throw logMessage;
				}
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] vDUT_Run(INSERT_DUT) return OK.\n");
			}
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] Fail to get vDut version information.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] Get vDut version information return OK.\n");
			}
		...
		else
		{
			// Registration failed
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] %s failed to register WIFI Technology.\n", l_insertDutParam.DUT_DLL_FILENAME);
		}
		...
		if (ERR_OK==err)
		{
			...
			vDUT_GetDoubleReturn(g_WiFi_Dut, "CURRENT", &l_insertDutReturn.CURRENT);	
    ...
    {
		ReturnErrorMessage(l_insertDutReturn.ERROR_MESSAGE, "[WiFi] Unknown Error!\n");
		err = -1;
    }
...
int InitializeInsertDutContainers(void)
{
    ...
    setting.unit        = "";
	setting.helpText    = "Specify the connection string for the DUT (i.e. ""SDIO"")";
	setting.type = WIFI_SETTING_TYPE_STRING;
	...
	l_insertDutReturn.CURRENT = NA_DOUBLE;
	setting.type = WIFI_SETTING_TYPE_DOUBLE;
	if (sizeof(double)==sizeof(l_insertDutReturn.CURRENT))    // Type_Checking
	{
		setting.value       = (void*)&l_insertDutReturn.CURRENT;
		setting.unit        = "mA";
		setting.helpText    = "Idle Current Measurement from external device.";
		l_insertDutReturnMap.insert( pair<string,WIFI_SETTING_STRUCT>("CURRENT", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
