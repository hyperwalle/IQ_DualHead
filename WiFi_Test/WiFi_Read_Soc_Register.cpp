...
#ifndef WIN32
int initReadSocRegisterContainers = InitializeReadSocRegisterContainers();
#endif
...
#pragma region Prepare input parameters
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] Prepare input parameters CheckReadSocRegisterParameters() return OK.\n");
		}
#pragma endregion	
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] vDUT_Run(READ_SOC_REGISTER) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] vDUT_Run(READ_SOC_REGISTER) return OK.\n");
		}
		...
		if ( ERR_OK!=err )
		{
			...
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] vDUT_GetStringReturn(SOC_REGISTER) return error.\n");
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] vDUT_GetStringReturn(SOC_REGISTER) return OK.\n");
		}
	...
	{
		ReturnErrorMessage(l_readSocRegisterReturn.ERROR_MESSAGE, "[WiFi] Unknown Error!\n");
	}
...
int CheckReadSocRegisterParameters(char *ADDRESS, int LENGTH, char* errorMsg)
{
	...
	try
	{
		...
		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[WiFi] CheckReadSocRegisterParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
