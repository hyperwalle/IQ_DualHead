...
#ifndef WIN32
int initWriteMacAddressContainers = InitializeWriteMacAddressContainers();
#endif
	...
	try
	{
		...
		*-----------------------------------------------------------*/
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi] vDUT_Run(WRITE_MAC_ADDRESS) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi] vDUT_Run(WRITE_MAC_ADDRESS) return OK.\n");
		}
