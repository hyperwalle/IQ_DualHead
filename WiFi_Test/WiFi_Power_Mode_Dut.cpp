...
struct tagParam
{
	...
	// TBD: After DUT control merged, should correct the spelling error CONTINOUS_TX_MODE to CONTINUOUS_TX_MODE
	int    CONTINOUS_TX_MODE;			            /*! Whether to send out continuous signal. */ // -cfy@sunnyvale, 2012/3/13-
...
#pragma region Prepare input parameters
		...
		err = CheckPathLossTable( 	g_WiFi_Test_ID, // -cfy@sunnyvale, 2012/3/13- 
...
//#pragma region Configure DUT to transmit
		...
		*---------------------------*/
		...
		// Keep for IQlite Backward Compatiable /* -cfy@sunnyvale, 2012/3/13- */
		vDUT_AddIntegerParameter(g_WiFi_Dut, "ATTEN1",				l_CurrentTestParam.ATTEN1);
		vDUT_AddIntegerParameter(g_WiFi_Dut, "ATTEN2",				l_CurrentTestParam.ATTEN2);
		vDUT_AddIntegerParameter(g_WiFi_Dut, "ATTEN3",				l_CurrentTestParam.ATTEN3);
		vDUT_AddIntegerParameter(g_WiFi_Dut, "ATTEN4",				l_CurrentTestParam.ATTEN4);
		/* <><~~ */
		...
		vDUT_AddIntegerParameter(g_WiFi_Dut, "CONTINOUS_TX_MODE",    l_CurrentTestParam.CONTINOUS_TX_MODE);
		/* <><~~ */
...
#pragma region Start RX CURRENT test
		   ...
		   &&(current_operation_mode == E_OPERATION_START))
	   {
		   	...
		   	// TBD: should call CheckDutTransmitStatus()
			//      and modify this function to input TX_TEST/RX_TEST parameter to do correct check
		    //      after that, we don't need to call "TX_STOP" here
			if ( g_vDutTxActived==true )
			{
			   /*-----------*
				*  Tx Stop  *
...
#pragma region Setup LP Tester
			...
			*---------------------*/
			...
			err = GetWaveformFileName(  "PER", 
										...
										l_CurrentTestParam.GUARD_INTERVAL_11N,
										modFile, 
										MAX_BUFFER_SIZE);
			if ( ERR_OK!=err )
			{
			...
			*--------------------*/
			...
			err = ::LP_SetVsg(l_CurrentTestParam.FREQ_MHZ*1e6, l_CurrentTestParam.POWER_DBM+l_CurrentTestParam.CABLE_LOSS_DB[antSelection], g_WiFiGlobalSettingParam.VSG_PORT);
			if ( ERR_OK!=err )
			{
...
int InitializePowerModeDutContainers(void)
{
	...
	*------------------*/
	...
	// -cfy@sunnyvale, 2012/3/13-
	l_CurrentTestParam.CONTINOUS_TX_MODE = 0;
	setting.type = WIFI_SETTING_TYPE_INTEGER;
	if (sizeof(int)==sizeof(l_CurrentTestParam.CONTINOUS_TX_MODE))    // Type_Checking
	{
		setting.value       = (void*)&l_CurrentTestParam.CONTINOUS_TX_MODE;
		setting.unit        = "";
		setting.helpText    = "Select to transmit continuous signal or not(0: with gap, 1: without gap).";
		l_CurrentTestParamMap.insert( pair<string,WIFI_SETTING_STRUCT>("CONTINOUS_TX_MODE", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
