...
int LP_initialize_dut(void)
{
	...
	try
	{
		...
		SendDutCmd("","wl rxchain 3");
		SendDutCmd("","wl txchain 3");
		SendDutCmd("","wl up");
		SendDutCmd("","wl down");
		SendDutCmd("","wl phy_percal 3");
		SendDutCmd("","wl phy_watchdog 0");
		...
		SendDutCmd("","wl plcphdr auto");
		SendDutCmd("","wl pkteng_start aa.bb.cc.dd.ee.ff tx sync 50 100 25");
		SendDutCmd("","wl pkteng_stop tx");
...
int LP_tx_set_bw(void)
{
	...
	try
	{
		...
		if((RxSameDut.nOldfreq == -1) || (g_channel<=14 && IsSameDut.nOldfreq > 14) || (g_channel>14 && IsSameDut.nOldfreq <= 14)) // 2g
		{			
			SendDutCmd(CmdPrefix,"mpc 0");
			SendDutCmd(CmdPrefix,"pkteng_stop tx");
			SendDutCmd(CmdPrefix,"down");
			SendDutCmd(CmdPrefix,"mimo_preamble 0"); // this command is used to control mixed or greenfield.
		...
		else
		{
			...
			sprintf_s(szCmd,sizeof(szCmd),"chanspec -c %d -b 5 -w %d -s %d",g_channel,g_nBandWidth,(g_nBandWidth/40));
			SendDutCmd(CmdPrefix,szCmd);	
		}

		/*SendDutCmd(CmdPrefix,"bi 65535");
		SendDutCmd(CmdPrefix,"up");	
		SendDutCmd(CmdPrefix,"txant 3");
...
int LP_tx_set_data_rate(void)
{
	...
	try
	{
		...
		else
		{
			...
			{
				map<string,string>::iterator it_rate = /*g_5grates*/g_rates.find(g_wifi_data_rate);
				assert(it_rate != g_rates.end());
...
int LP_tx_start(void)
{
	...
	try
	{
		...
		SendDutCmd(CmdPrefix,"pkteng_start aa.bb.cc.dd.ee.ff tx sync 50 100 25");
		SendDutCmd(CmdPrefix,"pkteng_stop tx");                                  
...
int LP_rx_set_bw(void)
{
	...
	else
	{
		...
		{
			SendDutCmd(CmdPrefix,"band a");			
			sprintf(szCmd,"chanspec -c %d -b 5 -w %d -s %d",g_channel,g_nBandWidth,g_nBandWidth/40);
			SendDutCmd(CmdPrefix,szCmd);	
			SendDutCmd(CmdPrefix,"up");
		}
		...
		case 1:
			...
			else
			{
				...
				SendDutCmd(CmdPrefix,"txchain 0x4");
			}
			break;	
		case 2:
			SendDutCmd(CmdPrefix,"rxchain 0x3");
