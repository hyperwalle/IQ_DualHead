...
#pragma region Prepare input parameters
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Prepare input parameters CheckWriteEepromParameters() return OK.\n");
		}
#pragma endregion
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] vDUT_Run(WRITE_EEPROM) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_Run(WRITE_EEPROM) return OK.\n");
		}
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_GetStringReturn(EEPROM_RETURN) return OK.\n");
		}
	...
	{
		ReturnErrorMessage(l_writeEepromReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
		err = -1;
	}
...
int CheckWriteEepromParameters(char *ADDRESS, char *VALUE, int LENGTH, char *FILE, char* errorMsg)
{
	...
	try
	{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Invalid FILE, ADDRESS, LENGTH or VALUE.\n");
			throw logMessage;
		}
		else
		{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Can't write file and write bytes in one item.\n");
			throw logMessage;
		}

		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[WiFi_11ac_MiMo] CheckWriteEepromParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
