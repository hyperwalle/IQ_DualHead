...
#pragma region Prepare input parameters
		...
		if ( ERR_OK!=err )
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Convert WiFi frequency to channel function failed.\n");
			throw logMessage;
		}
		else
		{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Invalid CURRENT TEST MODE %s.\n", l_CurrentTestParam.DUT_POWER_MODE);
			throw logMessage;		
		}
		else
		{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Invalid OPERATION MODE %s.\n", l_CurrentTestParam.DUT_OPERATION);
			throw logMessage;		
		}
		else
		{
...
//#pragma region Configure DUT to transmit
	   ...
	   if((current_test_mode == E_CURR_TEST_TX)&&(current_operation_mode == E_OPERATION_START))
	   {
		   ...
		   {
			   VHTMode = 0;   // 0: Normal 20MHz mode 
			   vDUT_AddIntegerParameter(g_WiFi_Dut, "FREQ_MHZ",			l_CurrentTestParam.FREQ_MHZ);
		   }		  
		   vDUT_AddStringParameter (g_WiFi_Dut, "PREAMBLE",				l_CurrentTestParam.PREAMBLE);
		   vDUT_AddStringParameter (g_WiFi_Dut, "PACKET_FORMAT",	l_CurrentTestParam.PACKET_FORMAT);
		   vDUT_AddStringParameter (g_WiFi_Dut, "GUARD_INTERVAL",		l_CurrentTestParam.GUARD_INTERVAL);
		   vDUT_AddStringParameter (g_WiFi_Dut, "STANDARD",		l_CurrentTestParam.STANDARD);
		   ...
		   vDUT_AddIntegerParameter(g_WiFi_Dut, "CHANNEL_BW",			VHTMode);
...
#pragma region Setup LP Tester
			...
			*----------------------------*/
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Fail to turn off VSG.\n");
				throw logMessage;
			}
			else
			{
			...
			*---------------------*/
			...
			err = GetWaveformFileName(  "PER", 
										...
										l_CurrentTestParam.PACKET_FORMAT, 
										modFile, 
										MAX_BUFFER_SIZE);
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Fail to get waveform file name.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Get waveform file name GetWaveformFileName() return OK.\n");
			}
			...
			*--------------------*/
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Fail to setup VSG.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Setup VSG LP_SetVsg() return OK.\n");
			}
			...
			*---------------*/
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Fail to turn on VSG.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Turn on LP_EnableVsgRF(1) return OK.\n");
			}
			...
			*-----------------------------*/           
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] VSG send out packet failed.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] VSG send out one packet for warm up LP_SetFrameCnt(1) return OK.\n");
			}
...
#pragma region Start RX receiving
			...
			*----------*/		
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Set VSG continious transmit LP_SetFrameCnt(0) failed.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Set VSG continious transmit LP_SetFrameCnt(0) return OK.\n");
			}
		}
#pragma endregion
...
#pragma region Stop RX CURRENT test
		...
		*-----------*/
		else if((current_test_mode == E_CURR_TEST_RX)&&(current_operation_mode == E_OPERATION_STOP))
		{
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Set VSG send out one packet LP_SetFrameCnt(1) failed.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Set VSG send out one packet LP_SetFrameCnt(1) return OK.\n");
			}
...
#pragma region START SLEEP CURRENT test
		 ...
		 {
			 err = -1;
			 LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] SLEEP MODE not implemented for this chip.\n");
			 throw logMessage;
		 }
#pragma endregion	
...
#pragma endregion	
	...
	{
		ReturnErrorMessage(l_CurrentTestReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
	}
...
int InitializePowerModeDutContainers(void)
{
	...
	*------------------*/
	...
	strcpy_s(l_CurrentTestParam.PACKET_FORMAT, MAX_BUFFER_SIZE, "MIXED");
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_CurrentTestParam.PACKET_FORMAT))    // Type_Checking
	{
		setting.value       = (void*)l_CurrentTestParam.PACKET_FORMAT;
		setting.unit        = "";
		setting.helpText    = "The packet format of 11N(only), can be MIXED or GREENFIELD, Default=MIXED.";
		l_CurrentTestParamMap.insert( pair<string,WIFI_SETTING_STRUCT>("PACKET_FORMAT", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
   ...
   // "STANDARD"
	strcpy_s(l_CurrentTestParam.STANDARD, MAX_BUFFER_SIZE, STANDARD_802_11_AC); 
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_CurrentTestParam.STANDARD))    // Type_Checking
	{
		setting.value       = (void*)l_CurrentTestParam.STANDARD;
		setting.unit        = "";
		...
		l_CurrentTestParamMap.insert( pair<string,WIFI_SETTING_STRUCT>("STANDARD", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	strcpy_s(l_CurrentTestParam.GUARD_INTERVAL, MAX_BUFFER_SIZE, "LONG");
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_CurrentTestParam.GUARD_INTERVAL))    // Type_Checking
	{
		setting.value       = (void*)l_CurrentTestParam.GUARD_INTERVAL;
		setting.unit        = "";
		setting.helpText    = "Packet Guard Interval, Long or Short, default is Long";
		l_CurrentTestParamMap.insert( pair<string,WIFI_SETTING_STRUCT>("GUARD_INTERVAL", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
