...
WIFI_11AC_MIMO_TEST_API int WiFi_Write_Mac_Address(void)
{
	...
	try
	{
		...
		*-----------------------------------------------------------*/
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] vDUT_Run(WRITE_MAC_ADDR) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_Run(WRITE_MAC_ADDR) return error.\n");
		}
    ...
    {
		ReturnErrorMessage(l_writeMacAddressReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
		err = -1;
    }
