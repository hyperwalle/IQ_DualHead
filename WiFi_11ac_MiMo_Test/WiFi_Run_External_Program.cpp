...
#ifndef WIN32
int initRunExternalProgramMIMOContainers = InitializeRunExternalProgramContainers();
#endif
...
#pragma region Prepare input parameters
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Prepare input parameters CheckWriteMacRegisterParameters() return OK.\n");
		}
#pragma endregion
...
#pragma region Execute external program
		...
		if(strlen(l_runExternalProgramParam.PROGRAM_WORKING_DIRECTORY)>1)
		{
			...
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Current Working Directory:[%s]", previousWorkingDirectory);
			...
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Switch Working Directory to [%s]", currentWorkingDirectory);
		}
		else
		{
			//do nothing
...
#pragma region Read result and return
		...
		if (strlen(l_runExternalProgramParam.RESULT_FILE_NAME)>1)
		{
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Could not open the result file.\n");
				err = 6;
				throw logMessage;
			}
			while(!feof(log))
			...
			if (!isLitePointDataFormat)
			{
				...
				if (iSize<MAX_BUFFER_SIZE)
				{
					...
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Incorrect result file format.\n");
					err = 6;
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] The result file exceed max length(4096 bytes).\n");
					err = 6;
					throw logMessage;
				}
			}
			else
			{
				...
				if (strlen(versionMap[tempStr].c_str())>0)
				{
					...
					{
						LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] The external program failed.\n");
						err = -1;
						throw logMessage;
					}
				}				
				//strcpy_s(l_runExternalProgramReturn.OUTPUT_MESSAGE, "[WiFi_11ac_MiMo] The results had been extracted.\n");
			}
		}
#pragma endregion	
		...
		if(strlen(l_runExternalProgramParam.PROGRAM_WORKING_DIRECTORY)>1)
		{
			...
			_getcwd(currentWorkingDirectory, MAX_BUFFER_SIZE);
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Current Working Directory to [%s]", currentWorkingDirectory);
		}
		else
		{
			//do nothing
    ...
    {
		ReturnErrorMessage(l_runExternalProgramReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
		err = -1;
    }
...
int CheckRunExternalProgramParameters(char* PROGRAM_NAME, char* ARGUMENTS, char* RESULT_FILE_NAME, int WAIT_TIME_MS, char* errorMsg)
{
	...
	try
	{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Invalid PROGRAM_NAME.\n");
			throw logMessage;
		}
		else
		{
		...
		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[WiFi_11ac_MiMo] CheckRunExternalProgramParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
