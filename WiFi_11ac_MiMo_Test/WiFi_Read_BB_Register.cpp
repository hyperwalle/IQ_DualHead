...
#ifndef WIN32
int initReadBBRegisterMIMOContainers = InitializeReadBBRegisterContainers();
#endif
...
#pragma region Prepare input parameters
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Prepare input parameters CheckReadBBRegisterParameters() return OK.\n");
		}
#pragma endregion	
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] vDUT_Run(READ_BB_REGISTER) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_Run(READ_BB_REGISTER) return OK.\n");
		}
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_GetStringReturn(BB_REGISTER) return OK.\n");
		}
    ...
    {
		ReturnErrorMessage(l_readBBRegisterReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
    }
...
int CheckReadBBRegisterParameters(char *ADDRESS, int LENGTH, char* errorMsg)
{
	...
	try
	{
		...
		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[WiFi_11ac_MiMo] CheckReadBBRegisterParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
