...
WIFI_11AC_MIMO_TEST_API int WiFi_Disconnect_IQTester(void)
{
	...
	try
	{
		...
		*---------------------------------------*/		
        ...
        if( g_Tester_Reconnect && 0!=g_Tester_Number )
        {
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Fail to terminate IQTester.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] LP_Term() successful.\n");
			} 
        }
        else
        {
			...
			{
				err = -1;
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] No IQtester is connected.\n");
				throw logMessage;
			}
        }
    ...
    {
		ReturnErrorMessage(l_disconnectTesterReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
		err = -1;
    }
