...
#pragma region Prepare input parameters
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Prepare input parameters CheckReadEepromParameters() return OK.\n");
		}
#pragma endregion	
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] vDUT_Run(READ_EEPROM) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_Run(READ_EEPROM) return OK.\n");
		}

		err = ::vDUT_GetStringReturn(g_WiFi_Dut, "EEPROM_RETURN", l_readEepromReturn.EEPROM_RETURN, MAX_BUFFER_SIZE);
		if ( ERR_OK!=err )
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] No Return, Unknown EEPROM read to address result.\n");
			throw logMessage;
		}
		else
		{
	...
	{
		ReturnErrorMessage(l_readEepromReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
		err = -1;
	}
...
int CheckReadEepromParameters(char *ADDRESS, int LENGTH, char *FILE, char* errorMsg)
{
	...
	try
	{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Invalid FILE, ADDRESS or LENGTH.\n");
			throw logMessage;
		}
		else
		{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Can't read file and read bytes in one item.\n");
			throw logMessage;
		}

		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[WiFi_11ac_MiMo] CheckReadEepromParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
