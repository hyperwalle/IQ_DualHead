...
WIFI_11AC_MIMO_TEST_API int WiFi_Remove_Dut(void)
{
	...
	try
	{
		...
		*-----------------------------------------------------------*/
		...
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] vDUT_Run(REMOVE_DUT) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] vDUT_Run(REMOVE_DUT) return OK.\n");
		}
		...
		if( 0!=g_ReloadDutDll && -1!=g_WiFi_Dut )
		{
			 ...
			 *****************************************************************************/
			err = ::vDUT_UnregisterDutDll("WIFI_11AC", &g_WiFi_Dut);
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11ac_MiMo] Unload the DUT control DLL from memory vDUT_UnregisterDutDll() return error.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11ac_MiMo] Unload the DUT control DLL from memory vDUT_UnregisterDutDll() return OK.\n");
			}
		}
		else
		{
    ...
    {
		ReturnErrorMessage(l_removeMIMODutReturn.ERROR_MESSAGE, "[WiFi_11ac_MiMo] Unknown Error!\n");
		err = -1;
    }
