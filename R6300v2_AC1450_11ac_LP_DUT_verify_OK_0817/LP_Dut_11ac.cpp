...
const char  *g_LP_Dut_Version                   = "1.5.2 (2013-08-19)\n";
...
int LP_remove_dut(void)
{
	...
	try 
	{
		...
		if(strstr(szDisablewf,"1") && strstr(szDisablewf1,"1"))
		{
			...
			GetPrivateProfileString("DISWIFI","cmd5","",stDisableWiFi.szCmd5,sizeof(stDisableWiFi.szCmd5),".\\LP_DUT_setup.ini");
			...
			SendDutCmd("",stDisableWiFi.szCmd5);
		} 
		//End
...
int LP_initialize_dut(void)
{
	...
	try
	{
		...
		// run these commands once for 5g.
		...
		SendDutCmd("","wl pkteng_stop tx"); // stop 2.4g tx
#else
...
int LP_tx_start(void)
{
	...
	try
	{
		...
		if(g_channel>14)
		{			
			SendDutCmd(CmdPrefix,"pkteng_start aa.bb.cc.dd.ee.ff tx sync 50 100 25"); // modify Packet IFS from 100 to 25
			SendDutCmd(CmdPrefix,"pkteng_stop tx");
			SendDutCmd(CmdPrefix,"phy_scraminit 127");
			SendDutCmd(CmdPrefix,"pkteng_start 00:00:00:c0:ff:ee tx 25 1740 0");		 
		}
		else
		{			
		...
		//SendDutCmd(CmdPrefix,"pkteng_start aa.bb.cc.dd.ee.ff tx sync 50 100 300");
		//SendDutCmd(CmdPrefix,"pkteng_stop tx");
		//SendDutCmd(CmdPrefix,"pkteng_start 00:00:00:c0:ff:ee tx 300 1740 0");
