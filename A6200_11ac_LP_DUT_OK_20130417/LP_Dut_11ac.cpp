...
map <string,string> g_rates,g_acrates;
...
int LP_tx_set_bw(void)
{
	...
	try
	{
		...
		{
			SendDutCmd("wl band a");
		}
		//SendDutCmd("wl chanspec 6/20");
		g_nBandWidth = int(pow(dBase,g_CH_bandwidth+1)*10); // nBandWidth is global
		if(g_nBandWidth == 80)
		{
			sprintf(szCmd,"wl chanspec %d/%d",g_channel-2,int(pow(dBase,g_CH_bandwidth+1)*10));
		}
		else
		{
			sprintf(szCmd,"wl chanspec %d/%d",g_channel,int(pow(dBase,g_CH_bandwidth+1)*10));
		}
		SendDutCmd(/*CmdPrefix,*/szCmd,"Chanspec set to 0x",5000);
		SendDutCmd("wl up");
//# wl -i eth2 up
...
int LP_tx_set_data_rate(void)
{
	...
	try
	{
		...
		if(g_channel<=14)
		{
			...
			SendDutCmd(/*CmdPrefix,*/"wl 2g_rate auto");
			SendDutCmd(/*CmdPrefix,*/"wl phy_forcecal 1");
			sprintf_s(szCmd,sizeof(szCmd),"wl 2g_rate %s" , it_rate->second.c_str());
	...
	try
	{
		...
		if(strstr(g_wifi_data_rate,"MCS"))
		{
			...
			case 1:
				if(g_currentAnt1)
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -s 1 -c 0x1");	
				}
				else if(g_currentAnt2)
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -s 1 -c 0x2");				
				}
				else if(g_currentAnt3)
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -s 1 -c 0x4");	
				}
				else
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -s 1 -c 0x8");	
				}
				break;
			case 2:
				nAntNo = GetAntCombineNum(g_currentAnt1,g_currentAnt2,g_currentAnt3,g_currentAnt4);
				sprintf_s(szCmd,sizeof(szCmd),"wl txcore -s 2 -c 0x%d",nAntNo);
				SendDutCmd(szCmd);
				//reseverd.
				break;
			case 3:
				nAntNo = GetAntCombineNum(g_currentAnt1,g_currentAnt2,g_currentAnt3,g_currentAnt4);
				sprintf_s(szCmd,sizeof(szCmd),"wl txcore -s 3 -c 0x%d",nAntNo);
				SendDutCmd(szCmd);
				//reseverd.
				break;
			case 4:
				nAntNo = GetAntCombineNum(g_currentAnt1,g_currentAnt2,g_currentAnt3,g_currentAnt4);
				sprintf_s(szCmd,sizeof(szCmd),"wl txcore -s 4 -c 0x%d",nAntNo);
				SendDutCmd(szCmd);
				//reseverd.
				break;
			default:
		...
		else	// OFDM, DSSS, support one ant only.
		{
			switch (nAntNum)
			{
			case 1:
				if(g_currentAnt1)
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -o 0x1");	
				}
				else if(g_currentAnt2)
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -o 0x2");				
				}
				else if(g_currentAnt3)
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -o 0x4");	
				}
				else
				{
					SendDutCmd(/*CmdPrefix,*/"wl txcore -o 0x8");	
				}
				break;
			case 2:
				//reseverd.
		...
		SendDutCmd("wl phy_scraminit 127");
		strset(szCmd,'\0');
		sprintf_s(szCmd,"wl txpwr1 -o -q %.1f", g_wifi_tx_power*4);
		SendDutCmd(/*CmdPrefix, */szCmd);
	}
	catch(char *msg)
    {
...
int LP_tx_start(void)
{
	...
	try
	{
		...
		SendDutCmd("wl pkteng_start 00:00:00:c0:ff:ee tx 250 1740 0");
...
int LP_tx_stop(void)
{
	...
	try
	{
		...
		*----------------------------*/
		...
		// Here add my codes
		SendDutCmd(/*CmdPrefix,*/"wl pkteng_stop tx");
	}
	catch(char *msg)
    {
        vDUT_AddStringReturn(g_LP_DUT_11ac_id, "ERROR_MESSAGE", msg);
	...
	else
	{
		...
		int nAntNum=g_currentAnt1+g_currentAnt2+g_currentAnt3+g_currentAnt4;
			...
			case 1:
				if(g_currentAnt1)
				{
					SendDutCmd(/*CmdPrefix,*/"wl rxchain 0x1");	
				}
				else if(g_currentAnt2)
				{
					SendDutCmd(/*CmdPrefix,*/"wl rxchain 0x2");				
				}
				else if(g_currentAnt3)
				{
					SendDutCmd(/*CmdPrefix,*/"wl rxchain 0x4");	
				}
				else
				{
					SendDutCmd(/*CmdPrefix,*/"wl rxchain 0x8");	
				}
				break;
			case 2:
				nAntNo = GetAntCombineNum(g_currentAnt1,g_currentAnt2,g_currentAnt3,g_currentAnt4);
				sprintf_s(szCmd,sizeof(szCmd),"wl rxchain 0x%d",nAntNo);
				SendDutCmd(szCmd);
				//reseverd.
				break;
			case 3:
...
int LP_clear_rx_pckts(void)
{
	...
	else
	{
		...
		SendDutCmd(/*CmdPrefix,*/"wl counters","currchainsz",5000);

		int index = (int)strSocketBuf.find("rxstrt");
		...
		else
		{
			...
			const size_t size =  strlen("rxstrt ");
