...
map<string, WIFI_SETTING_STRUCT> l_11ACwriteMacRegisterParamMap;

// Return Value Container 
map<string, WIFI_SETTING_STRUCT> l_11ACwriteMacRegisterReturnMap;
...
} l_11ACwriteMacRegisterReturn;
#pragma endregion

#ifndef WIN32
int init11ACWriteMacRegisterContainers = Initialize11ACWriteMacRegisterContainers();
#endif

// These global variables/functions only for WiFi_Write_Mac_Register.cpp
int Check11ACWriteMacRegisterParameters(char *ADDRESS, char *VALUE, char* errorMsg);
...
WIFI_11AC_TEST_API int WIFI_11AC_Write_Mac_Register(void)
{
	...
	*---------------------------------------*/
	ClearReturnParameters(l_11ACwriteMacRegisterReturnMap);
	...
	*------------------------*/
	...
	if( ERR_OK==err )
	{
		RespondToQueryInput(l_11ACwriteMacRegisterParamMap);
		return err;
	}
	else
	{
	...
	*-------------------------*/
	...
	if( ERR_OK==err )
	{
		RespondToQueryReturn(l_11ACwriteMacRegisterReturnMap);
		return err;
	}
	else
	{
	...
	try
	{
		...
		*----------------------*/
		err = GetInputParameters(l_11ACwriteMacRegisterParamMap);
		if ( ERR_OK!=err )
		{
		...
		// WRITE_COMMAND
		vDUT_AddStringParameter (g_WiFi_11ac_Dut,  "ADDRESS",   l_11ACwriteMacRegisterParam.ADDRESS);
		vDUT_AddStringParameter (g_WiFi_11ac_Dut,  "VALUE",     l_11ACwriteMacRegisterParam.VALUE);

#pragma region Prepare input parameters
		err = Check11ACWriteMacRegisterParameters( l_11ACwriteMacRegisterParam.ADDRESS, l_11ACwriteMacRegisterParam.VALUE, vErrorMsg );
		if ( ERR_OK!=err )
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, vErrorMsg);
			throw logMessage;
...
#pragma endregion
		...
		err = vDUT_Run(g_WiFi_11ac_Dut, "WRITE_MAC_REGISTER");		
		if ( ERR_OK!=err )
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			err = ::vDUT_GetStringReturn(g_WiFi_11ac_Dut, "ERROR_MESSAGE", vErrorMsg, MAX_BUFFER_SIZE);
			if ( ERR_OK==err )	// Get "ERROR_MESSAGE" from vDut
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WiFi_11AC] vDUT_Run(WRITE_MAC_REGISTER) return error.\n");
				throw logMessage;
			}
		}
		else
		{		
			...
			err = ::vDUT_GetStringReturn(g_WiFi_11ac_Dut, "WRITE_REGISTER", l_11ACwriteMacRegisterReturn.WRITE_REGISTER, MAX_BUFFER_SIZE);
			if ( ERR_OK!=err )
			{
			...
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WiFi_11AC] vDUT_Run(WRITE_MAC_REGISTER) return OK.\n");
		}
		...
		*-----------------------*/
		if (ERR_OK==err)
		{
			sprintf_s(l_11ACwriteMacRegisterReturn.ERROR_MESSAGE, MAX_BUFFER_SIZE, "[Info] Function completed.\n");
			ReturnTestResults(l_11ACwriteMacRegisterReturnMap);
		}
		else
		{
			// do nothing
	...
	{
		ReturnErrorMessage(l_11ACwriteMacRegisterReturn.ERROR_MESSAGE, msg);
	}
	catch(...)
	{
		ReturnErrorMessage(l_11ACwriteMacRegisterReturn.ERROR_MESSAGE, "[WiFi_11AC] Unknown Error!\n");
	}

	return err;
}

int Initialize11ACWriteMacRegisterContainers(void)
{
	/*------------------*
	* Input Parameters: *
	*------------------*/
	l_11ACwriteMacRegisterParamMap.clear();

	WIFI_SETTING_STRUCT setting;

	sprintf_s(l_11ACwriteMacRegisterParam.ADDRESS, MAX_BUFFER_SIZE, "%s", "");
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_11ACwriteMacRegisterParam.ADDRESS))    // Type_Checking
	{
		setting.value = (void*)l_11ACwriteMacRegisterParam.ADDRESS;
		setting.unit        = "";
		setting.helpText    = "The address that is used to write Mac Register.";
		l_11ACwriteMacRegisterParamMap.insert( pair<string,WIFI_SETTING_STRUCT>("ADDRESS", setting) );
	}
	else    
	{
	...
	sprintf_s(l_11ACwriteMacRegisterParam.VALUE, MAX_BUFFER_SIZE, "%s", "");
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_11ACwriteMacRegisterParam.VALUE))    // Type_Checking
	{
		setting.value = (void*)l_11ACwriteMacRegisterParam.VALUE;
		setting.unit        = "";
		setting.helpText    = "The value that is used to write Mac Register.";
		l_11ACwriteMacRegisterParamMap.insert( pair<string,WIFI_SETTING_STRUCT>("VALUE", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	* ERROR_MESSAGE  *
	*----------------*/
	l_11ACwriteMacRegisterReturnMap.clear();

	// TODO: Example, add Return parameters here
	l_11ACwriteMacRegisterReturn.WRITE_REGISTER[0] = '\0';
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_11ACwriteMacRegisterReturn.WRITE_REGISTER))    // Type_Checking
	{
		setting.value       = (void*)l_11ACwriteMacRegisterReturn.WRITE_REGISTER;
		setting.unit        = "";
		setting.helpText    = "The value had been written to BB Register.";
		l_11ACwriteMacRegisterReturnMap.insert( pair<string,WIFI_SETTING_STRUCT>("WRITE_REGISTER", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	l_11ACwriteMacRegisterReturn.ERROR_MESSAGE[0] = '\0';
	setting.type = WIFI_SETTING_TYPE_STRING;
	if (MAX_BUFFER_SIZE==sizeof(l_11ACwriteMacRegisterReturn.ERROR_MESSAGE))    // Type_Checking
	{
		setting.value       = (void*)l_11ACwriteMacRegisterReturn.ERROR_MESSAGE;
		setting.unit        = "";
		setting.helpText    = "Error message occurred";
		l_11ACwriteMacRegisterReturnMap.insert( pair<string,WIFI_SETTING_STRUCT>("ERROR_MESSAGE", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
...
int Check11ACWriteMacRegisterParameters(char *ADDRESS, char *VALUE, char* errorMsg)
{
	...
	try
	{
		...
		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[WiFi_11AC] CheckWriteMacRegisterParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
