...
int LP_initialize_dut(void)
{
	...
	try
	{
		...
		else
		{
			...
			//set data rate
			SendDutCmd("","wl -i wl0 nrate auto");
			SendDutCmd("","wl -i wl1 nrate auto");
		}
...
int LP_tx_set_antenna(void)
{
	...
	try
	{
		...
		if(g_channel <= 14)//2G channel
		{
			//set DUT channel
			strset(szCmd,'\0');
			sprintf_s(szCmd,"wl -i wl0 channel %d", g_channel);	
		}
		else //5G channel
		{
			//set DUT channel
			strset(szCmd,'\0');
			sprintf_s(szCmd,"wl -i wl1 channel %d", g_channel);
			SendDutCmd("", szCmd);					
		}
