...
map<string, GPS_SETTING_STRUCT> l_disconnectTesterParamMap;

// Return Value Container 
map<string, GPS_SETTING_STRUCT> l_disconnectTesterReturnMap;

// GPS_Disconnect_IQTester does not require any inputs
...
GPS_TEST_API int GPS_Disconnect_IQTester(void)
{
	...
	try
	{
		...
		*---------------------------------------*/
        ...
        if( g_Tester_Reconnect && 0!=g_Tester_Number )
        {
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[GPS] Fail to terminate IQTester.\n");
				throw logMessage;
			}
        }
        else
    ...
    {
		ReturnErrorMessage(l_disconnectTesterReturn.ERROR_MESSAGE, "[GPS] Unknown Error!\n");
		err = -1;
    }
...
void InitializeDisconnectTesterContainers(void)
{
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_disconnectTesterReturn.ERROR_MESSAGE))    // Type_Checking
    {
        ...
        l_disconnectTesterReturnMap.insert( pair<string,GPS_SETTING_STRUCT>("ERROR_MESSAGE", setting) );
    }
    else    
    {
