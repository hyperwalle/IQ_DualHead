...
map<string, BT_SETTING_STRUCT> l_runExternalProgramParamMap;

// Return Value Container 
map<string, BT_SETTING_STRUCT> l_runExternalProgramReturnMap;
...
// These global variables/functions only for BT_Run_External_Program.cpp
...
#pragma region Prepare input parameters
		...
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[BT] Prepare input parameters CheckWriteMacRegisterParameters() return OK.\n");
		}
#pragma endregion
...
#pragma region Read result and return
		...
		if (strlen(l_runExternalProgramParam.RESULT_FILE_NAME)>1)
		{
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[BT] Could not open the result file.\n");
				err = 6;
				throw logMessage;
			}
			while(!feof(log))
			...
			if (!isLitePointDataFormat)
			{
				...
				if (iSize<MAX_BUFFER_SIZE)
				{
					...
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[BT] Incorrect result file format.\n");
					err = 6;
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[BT] The result file exceed max length(4096 bytes).\n");
					err = 6;
					throw logMessage;
				}
			}
			else
			{
				...
				//strcpy_s(l_runExternalProgramReturn.OUTPUT_MESSAGE, "[BT] The results had been extracted.\n");
			}
		}
#pragma endregion	
		/*-----------------------*
	...
	{
		ReturnErrorMessage(l_runExternalProgramReturn.ERROR_MESSAGE, "[BT] Unknown Error!\n");
	}
...
int InitializeRunExternalProgramContainers(void)
{
	...
	*------------------*/
	...
	if (MAX_BUFFER_SIZE==sizeof(l_runExternalProgramParam.PROGRAM_NAME))    // Type_Checking
	{
		...
		l_runExternalProgramParamMap.insert( pair<string,BT_SETTING_STRUCT>("PROGRAM_NAME", setting) );
	}
	else    
	{
	...
	if (MAX_BUFFER_SIZE==sizeof(l_runExternalProgramParam.ARGUMENTS))    // Type_Checking
	{
		...
		l_runExternalProgramParamMap.insert( pair<string,BT_SETTING_STRUCT>("ARGUMENTS", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	if (MAX_BUFFER_SIZE==sizeof(l_runExternalProgramParam.RESULT_FILE_NAME))    // Type_Checking
	{
		...
		l_runExternalProgramParamMap.insert( pair<string,BT_SETTING_STRUCT>("RESULT_FILE_NAME", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	if (sizeof(int)==sizeof(l_runExternalProgramParam.WAIT_TIME_MS))    // Type_Checking
	{
		...
		l_runExternalProgramParamMap.insert( pair<string,BT_SETTING_STRUCT>("WAIT_TIME_MS", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	*----------------*/
	...
	if (MAX_BUFFER_SIZE==sizeof(l_runExternalProgramReturn.ERROR_MESSAGE))    // Type_Checking
	{
		...
		l_runExternalProgramReturnMap.insert( pair<string,BT_SETTING_STRUCT>("ERROR_MESSAGE", setting) );
	}
	else    
	{
		printf("Parameter Type Error!\n");
	...
	//    l_runExternalProgramReturnMap.insert( pair<string,BT_SETTING_STRUCT>("OUTPUT_MESSAGE", setting) );
	//}
	//else    
	//{
	//    printf("Parameter Type Error!\n");
	...
	for (int i=0; i<MAX_RESULT_GROUP_NUM; i++)
	{
		...
		if (sizeof(double)==sizeof(l_runExternalProgramReturn.CURRENT_MA[i]))    // Type_Checking
		{
			...
			l_runExternalProgramReturnMap.insert( pair<string,BT_SETTING_STRUCT>(tempStr, setting) );
		}
		else    
		{
			printf("Parameter Type Error!\n");
...
//	int    dummyInt = 0;
	...
	try
	{
		...
		{
			err = -1;
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[BT] Invalid PROGRAM_NAME.\n");
			throw logMessage;
		}
		else
		{
		...
		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[BT] CheckRunExternalProgramParameters() Confirmed.\n");
	}
	catch(char *msg)
	{
	...
	{
		sprintf_s(errorMsg, MAX_BUFFER_SIZE, "[BT] Unknown Error!\n");
	}
