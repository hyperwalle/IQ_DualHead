...
map<string, WIMAX_SETTING_STRUCT> l_readMacAddressParamMap;

// Return Value Container 
map<string, WIMAX_SETTING_STRUCT> l_readMacAddressReturnMap;
...
WIMAX_TEST_API int WIMAX_Read_Mac_Address(void)
{
	...
	try
	{
		...
		*-----------------------------------------------------------*/
		...
		err = vDUT_Run(g_WIMAX_Dut, "READ_MAC_ADDRESS");		
		if ( ERR_OK!=err )
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] vDUT_Run(READ_MAC_ADDRESS) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] vDUT_Run(READ_MAC_ADDRESS) return OK.\n");
		}

		// TODO: Example, get Return parameters here
		err = ::vDUT_GetStringReturn(g_WIMAX_Dut, "MAC_ADDRESS", l_readMacAddressReturn.MAC_ADDRESS, MAX_BUFFER_SIZE);		
		if ( ERR_OK!=err )
		{
		...
		else
		{
			...
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] vDUT_GetStringReturn(MAC_ADDRESS=%s) return OK.\n",l_readMacAddressReturn.MAC_ADDRESS);
		}
    ...
    {
		ReturnErrorMessage(l_readMacAddressReturn.ERROR_MESSAGE, "[WIMAX] Unknown Error!\n");
		err = -1;
    }
...
void InitializeReadMacAddressContainers(void)
{
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_readMacAddressReturn.MAC_ADDRESS))    // Type_Checking
    {
        ...
        l_readMacAddressReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("MAC_ADDRESS", setting) );
    }
    else    
    {
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_readMacAddressReturn.ERROR_MESSAGE))    // Type_Checking
    {
        ...
        l_readMacAddressReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("ERROR_MESSAGE", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
