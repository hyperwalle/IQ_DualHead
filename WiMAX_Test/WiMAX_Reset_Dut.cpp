...
map<string, WIMAX_SETTING_STRUCT> l_resetDutParamMap;

// Return Value Container 
map<string, WIMAX_SETTING_STRUCT> l_resetDutReturnMap;
...
WIMAX_TEST_API int WIMAX_Reset_Dut(void)
{
	...
	try
	{
		...
		*-----------------------------------------------------------*/
        ...
        // RESET_DUT
		err = ::vDUT_Run(g_WIMAX_Dut, "RESET_DUT");		
		if ( ERR_OK!=err )
		{	// Check vDut return "ERROR_MESSAGE" or not, if "Yes", must handle it.
			...
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] vDUT_Run(RESET_DUT) return error.\n");
				throw logMessage;
			}
		}
		else
		{
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] vDUT_Run(RESET_DUT) return OK.\n");
		}
    ...
    {
		ReturnErrorMessage(l_resetDutReturn.ERROR_MESSAGE, "[WIMAX] Unknown Error!\n");
		err = -1;
    }
...
void InitializeResetDutContainers(void)
{
    /*------------------*
     * Input Parameters: *
     * IQTESTER_IP01    *
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_resetDutReturn.ERROR_MESSAGE))    // Type_Checking
    {
        ...
        l_resetDutReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("ERROR_MESSAGE", setting) );
    }
    else    
    {
