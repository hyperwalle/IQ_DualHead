...
map<string, WIMAX_SETTING_STRUCT> l_connectTesterParamMap;

// Return Value Container 
map<string, WIMAX_SETTING_STRUCT> l_connectTesterReturnMap;
...
WIMAX_TEST_API int WIMAX_Connect_IQTester(void)
{
	...
	try
	{
		...
		*---------------------------------------*/
        ...
        if( (0==g_Tester_Number) || g_Tester_Reconnect )
		{
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Invalid IP address (%s).\n", l_connectTesterParam.IQTESTER_IP01);
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] IP address = (%s).\n", l_connectTesterParam.IQTESTER_IP01);
			}
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] IQapi LP_Init() Failed.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] IQapi LP_Init() return OK.\n");
			}
                 ...
                 (ERR_OK!=ValidateIPAddress(l_connectTesterParam.IQTESTER_IP04)) )
            {
				...
				if ( ERR_OK!=err )
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Function LP_InitTester(%s) Failed.\n", l_connectTesterParam.IQTESTER_IP01);
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] Function LP_InitTester(%s) return OK.\n", l_connectTesterParam.IQTESTER_IP01);
				}
                g_Tester_Type   = IQ_MAX;
                g_Tester_Number = 1;
            }
					  ...
					  (ERR_OK!=ValidateIPAddress(l_connectTesterParam.IQTESTER_IP04)) )
            {
				...
				if ( ERR_OK!=err )
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Function LP_InitTester2(%s, %s) Failed.\n", l_connectTesterParam.IQTESTER_IP01, l_connectTesterParam.IQTESTER_IP02);
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] Function LP_InitTester2(%s, %s) return OK.\n", l_connectTesterParam.IQTESTER_IP01, l_connectTesterParam.IQTESTER_IP02);
				}
                g_Tester_Type   = IQ_nxn_Plus;
                g_Tester_Number = 2;
            }
            else if ( (ERR_OK==ValidateIPAddress(l_connectTesterParam.IQTESTER_IP02))&&
					  (ERR_OK==ValidateIPAddress(l_connectTesterParam.IQTESTER_IP03))&&
					  (ERR_OK!=ValidateIPAddress(l_connectTesterParam.IQTESTER_IP04)) )
            {
				...
				if ( ERR_OK!=err )
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Function LP_InitTester3(%s, %s, %s) Failed.\n", l_connectTesterParam.IQTESTER_IP01, l_connectTesterParam.IQTESTER_IP02, l_connectTesterParam.IQTESTER_IP03);
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] Function LP_InitTester3(%s, %s, %s) return OK.\n", l_connectTesterParam.IQTESTER_IP01, l_connectTesterParam.IQTESTER_IP02, l_connectTesterParam.IQTESTER_IP03);
				}                
				g_Tester_Type   = IQ_nxn_Plus;
                g_Tester_Number = 3;
            }
					  ...
					  (ERR_OK==ValidateIPAddress(l_connectTesterParam.IQTESTER_IP04)) )
            {
				...
				if ( ERR_OK!=err )
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Function LP_InitTester4(%s, %s, %s, %s) Failed.\n", l_connectTesterParam.IQTESTER_IP01, l_connectTesterParam.IQTESTER_IP02, l_connectTesterParam.IQTESTER_IP03, l_connectTesterParam.IQTESTER_IP04);
					throw logMessage;
				}
				else
				{
					LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] Function LP_InitTester4(%s, %s, %s, %s) return OK.\n", l_connectTesterParam.IQTESTER_IP01, l_connectTesterParam.IQTESTER_IP02, l_connectTesterParam.IQTESTER_IP03, l_connectTesterParam.IQTESTER_IP04);
				}                 
				g_Tester_Type   = IQ_nxn_Plus;
                g_Tester_Number = 4;
            }
            else
            {
                ...
                err = -1;
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Invalid IP address sequence for IQnxn.\n");
				throw logMessage; 
            }
			
			LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] IQtester initiation accomplished.\n");

			// Call LP_SetVsa() for apply VSA and VSG port setting 
			err = ::LP_SetVsa(  2450*1e6, 
								0, 
            ...
            {
                err = -1;
                LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Failto get IQTester version information.\n");
				throw logMessage;
            }
            else
            {
                err = 0;
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] Get IQTester version information return OK.\n");
            }
            ...
            {
				l_IQ201xFound = true;
				if ( g_WIMAXGlobalSettingParam.VSA_TRIGGER_TYPE==6 )   g_WIMAXGlobalSettingParam.VSA_TRIGGER_TYPE = 13; //IQV_TRIG_TYPE_FOR_IQ2010
                if ( g_WIMAXGlobalSettingParam.VSA_TRIGGER_LEVEL_DB < -20)
				{
                    g_WIMAXGlobalSettingParam.VSA_TRIGGER_LEVEL_DB = -20;  //some 2010 cannot trigger with -25 dB, so -20 will be more safe with 2010
				}
				else
                {
			...
			if ( ERR_OK!=err )
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_ERROR, "[WIMAX] Fail to get TestManager version information.\n");
				throw logMessage;
			}
			else
			{
				LogReturnMessage(logMessage, MAX_BUFFER_SIZE, LOGGER_INFORMATION, "[WIMAX] Get TestManager version information return OK.\n");
			}   
        }
        else
        {
		...
		if ( l_IQ201xFound&&(g_WIMAXGlobalSettingParam.VSA_TRIGGER_TYPE==6) )
		{
			g_WIMAXGlobalSettingParam.VSA_TRIGGER_TYPE = 13;		//IQV_TRIG_TYPE_FOR_IQ2010
		}
		else
		{
			// do nothing
    ...
    {
		ReturnErrorMessage(l_connectTesterReturn.IQLITE_ERROR_MESSAGE, "[WIMAX] Unknown Error!\n");
		err = -1;
    }
...
void InitializeConnectTesterContainers(void)
{
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterParam.IQTESTER_IP01))    // Type_Checking
    {
        ...
        l_connectTesterParamMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_IP01", setting) );
    }
    else    
    {
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterParam.IQTESTER_IP02))    // Type_Checking
    {
        ...
        setting.unit        = "";
        setting.helpText    = "IP address of LitePoint tester";
        l_connectTesterParamMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_IP02", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterParam.IQTESTER_IP03))    // Type_Checking
    {
        ...
        setting.unit        = "";
        setting.helpText    = "IP address of LitePoint tester";
        l_connectTesterParamMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_IP03", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterParam.IQTESTER_IP04))    // Type_Checking
    {
        ...
        setting.unit        = "";
        setting.helpText    = "IP address of LitePoint tester";
        l_connectTesterParamMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_IP04", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
    ...
    if (sizeof(int)==sizeof(l_connectTesterParam.IQTESTER_RECONNECT))    // Type_Checking
    {
        ...
        l_connectTesterParamMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_RECONNECT", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQTESTER_INFO))    // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_INFO", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQTESTER_SERIAL_NUM_01))  // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_SERIAL_NUM_01", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQTESTER_SERIAL_NUM_02))  // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_SERIAL_NUM_02", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQTESTER_SERIAL_NUM_03))  // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_SERIAL_NUM_03", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQTESTER_SERIAL_NUM_04))  // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQTESTER_SERIAL_NUM_04", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQAPI_VERSION))   // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQAPI_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQAPITC_VERSION)) // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQAPITC_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQMEASURE_VERSION))   // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQMEASURE_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQV_ANALYSIS_VERSION))    // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQV_ANALYSIS_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQV_MW_VERSION))  // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQV_MW_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQ_NXN_VERSION))  // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQ_NXN_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQ_NXN_SIGNAL_VERSION))   // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQ_NXN_SIGNAL_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQ_MAX_SIGNAL_VERSION))   // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQ_MAX_SIGNAL_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_IQ_ZIGBEE_SIGNAL_VERSION))    // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("IQ_ZIGBEE_SIGNAL_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_BLUETOOTH_VERSION))   // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("BLUETOOTH_VERSION", setting) );
    }
    else
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_TEST_MANAGER_VERSION))    // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("TEST_MANAGER_VERSION", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
    ...
    if (MAX_BUFFER_SIZE==sizeof(l_connectTesterReturn.IQLITE_ERROR_MESSAGE))    // Type_Checking
    {
        ...
        l_connectTesterReturnMap.insert( pair<string,WIMAX_SETTING_STRUCT>("ERROR_MESSAGE", setting) );
    }
    else    
    {
        printf("Parameter Type Error!\n");
