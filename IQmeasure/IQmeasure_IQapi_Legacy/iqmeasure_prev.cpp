...
highrestimer::lp_time_t timeStart, timeStop; // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
double                  timeDuration;
double					g_testerHwVersion = 0;			// IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
bool					g_bDisableFreqFilter = true;	// IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
...
extern "C" int iqapiSetCompThreadNumber(iqapiHndl *hndl, int compThreadNumber, int *numberCompThreadSetLast); // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
...
int LP_FreeMemory(void)
{
	...
	if (FmInitialized == true)
		if (hndl->hndlFm) {hndl->hndlFm->analysis = NULL;} // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
...
IQMEASURE_API bool LP_GetVersion(char *buffer, int buf_size)
{
    ...
    sprintf_s(str, MAX_BUFFER_LEN, "IQmeasure: %s.%s %s\n", IQAPI_VERSION, IQMeasureVersion, SVN_REVISION); // Modified /* -cfy@sunnyvale, 2012/3/13- */
...
IQMEASURE_API int LP_Agc(double *rfAmplDb, bool allTesters) // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
{
	int err = ERR_OK;
	...
	if (LibsInitialized)
	{
		hndl->Agc(allTesters); // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
...
IQMEASURE_API int LP_SetVsgNxN(double rfFreqHz, double rfPowerLeveldBm[], int port[])
{
	...
	return LP_SetVsg(rfFreqHz, rfPowerLeveldBm[0], port[0]);
}


IQMEASURE_API int LP_SetVsg(double rfFreqHz, double rfPowerLeveldBm, int port, bool setGapPowerOff)
{
	int err = ERR_OK;
	bool setupChanged = false;
	bool RFused = true;
...
IQMEASURE_API int LP_SetVsg_GapPower(double rfFreqHz, double rfPowerLeveldBm, int port, int gapPowerOff)
{
	...
	if (LibsInitialized)
	{
		if ( (hndl->tx->rfFreqHz!=rfFreqHz)||(hndl->tx->freqShiftHz!=0)||g_bDisableFreqFilter == true ) // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
		{
			hndl->tx->rfFreqHz = rfFreqHz;
			hndl->rx->rfFreqHz = rfFreqHz;
			...
			hndl->rx->freqShiftHz = 0;
			setupChanged = true;
			g_bDisableFreqFilter = false; // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
		}
		if (hndl->tx->txMode == IQV_INPUT_MOD_DAC_BB && port != PORT_BB)
		{
			hndl->tx->txMode = IQV_INPUT_MOD_DAC_RF;
...
IQMEASURE_API int LP_SetVsaBluetooth(double rfFreqHz, double rfAmplDb, int port, double triggerLevelDb, double triggerPreTime)
{
	...
	if (LibsInitialized)
	{
		if (hndl->rx->rfFreqHz != (rfFreqHz+BT_SHIFT_HZ) || g_bDisableFreqFilter == true ) // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
		{
	...
	//		port = (int)PORT_LEFT;
		...
		for (int i=0; i<nTesters; i++)
		{
			...
			if (true == rfAmplChanged) // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
			{
				if(RFused)
							   ...
							   double triggerPreTime)
{
	...
	return LP_SetVsa(rfFreqHz, rfAmplDb[0], port[0], extAttenDb, triggerLevelDb, triggerPreTime);
}

IQMEASURE_API int LP_SetVsa(double rfFreqHz, double rfAmplDb, int port, double extAttenDb, double triggerLevelDb, double triggerPreTime)
{
	int err = ERR_OK;
	bool setupChanged = false;
	bool RFused = true;
	int  recordedTriggerType = 0; // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
	...
	if (LibsInitialized)
	{
		...
		// IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
		if ( (hndl->rx->rfFreqHz!=rfFreqHz)||(hndl->rx->freqShiftHz!=0)||(hndl->tx->rfFreqHz!=rfFreqHz) || g_bDisableFreqFilter == true)
		{
			hndl->rx->rfFreqHz = rfFreqHz;
			hndl->tx->rfFreqHz = rfFreqHz;
			hndl->rx->freqShiftHz = 0;
...
IQMEASURE_API int LP_VsaDataCapture(double samplingTimeSecs, int triggerType, double sampleFreqHz, int ht40Mode, IQMEASURE_CAPTURE_NONBLOCKING_STATES nonBlockingState )
{
	int err = ERR_OK;
	bool setupChanged = false;
	...
	if ( LibsInitialized )
	{
		...
		if(nonBlockingState!=ARM_TRIGGER) // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
		{
...
IQMEASURE_API int LP_Analyze80211ag(int ph_corr_mode, int ch_estimate, int sym_tim_corr, int freq_sync, int ampl_track)
{
	int err = ERR_OK;
...
IQMEASURE_API int LP_Analyze80211b(int eq_taps, int DCremove11b_flag, int method_11b)
{
	int err = ERR_OK;
...
//				}
			...
			else if (dynamic_cast<iqapiResult11b *>(hndl->results))			
			{
				...
				if (!strcmp(measurement, "plcpCrcFail"))
				{
				...
				if (!strcmp(measurement, "longPreamble"))
				{
					return((double)result11b->longPreamble);					
				}
				
				if (!strcmp(measurement, "bitRateInMHz"))
				{
...
#ifdef WIN32	//Not using for Mac at this time // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
	...
	if(FmInitialized)
	{
		try
		{
			...
			else
			{
				...
				hndl->hndlFm->SetParamPilotDeviation(0); // Change back from 1000 to 0 coz issue fixed. // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
			}
			if(IQV_FM_VSG_RDS_MODE_ON == rdsEnable)
			{
				...
				if(IQAPI_ERR_OK != err) throw err; // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
			...
			else
			{
				...
				hndl->hndlFm->SetParamRdsDeviation(0); // Change back from 1000 to 0 coz issue fixed. // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
			}
													...
													char* audioWaveFileOutput)
{
	...
	{
		err = ERR_FM_PLAY_AUDIO_FAILED; // IQfuji merge /* -cfy@sunnyvale, 2012/3/13- */
	}
...
IQMEASURE_API int LP_SetVsgModulation_SetPlayCondition(char *modFileName, bool autoPlay)
{
	int err = ERR_OK;
