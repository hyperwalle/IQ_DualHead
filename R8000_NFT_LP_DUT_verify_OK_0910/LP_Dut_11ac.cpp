...
const char  *g_LP_Dut_Version                   = "1.6.2 (2014-10-08)\n";
...
LP_DUT_11AC_API int WIFI_11AC_DutRegister(void)
{
	// add rates list
	g_rates.clear();
	g_rates.insert( pair<string, string>("DSSS-1",   "-r 1 -b 20") );
	g_rates.insert( pair<string, string>("DSSS-2",   "-r 2 -b 20") );
	g_rates.insert( pair<string, string>("CCK-5_5",  "-r 5.5 -b 20") );
	g_rates.insert( pair<string, string>("CCK-11",   "-r 11 -b 20") );
	g_rates.insert( pair<string, string>("PBCC-22",  "-r 22 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-6",   "-r 6 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-9",   "-r 9 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-12",  "-r 12 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-18",  "-r 18 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-24",  "-r 24 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-36", "-r 36 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-48", "-r 48 -b 20") );
	g_rates.insert( pair<string, string>("OFDM-54", "-r 54 -b 20") );
	g_rates.insert( pair<string, string>("MCS0",    "-h 0 -b %d") );
	g_rates.insert( pair<string, string>("MCS1",    "-h 1 -b %d") );
	g_rates.insert( pair<string, string>("MCS2",    "-h 2 -b %d") );
	g_rates.insert( pair<string, string>("MCS3",    "-h 3 -b %d") );
	g_rates.insert( pair<string, string>("MCS4",    "-h 4 -b %d") );
	g_rates.insert( pair<string, string>("MCS5",    "-h 5 -b %d") );
	g_rates.insert( pair<string, string>("MCS6",    "-h 6 -b %d") );
	g_rates.insert( pair<string, string>("MCS7",    "-h 7 -b %d") );
	g_rates.insert( pair<string, string>("MCS8",    "-h 8 -b %d") );
	g_rates.insert( pair<string, string>("MCS9",    "-h 9 -b %d") );
	g_rates.insert( pair<string, string>("MCS10",   "-h 10 -b %d") );
	g_rates.insert( pair<string, string>("MCS11",   "-h 11 -b %d") );
	g_rates.insert( pair<string, string>("MCS12",   "-h 12 -b %d") );
	g_rates.insert( pair<string, string>("MCS13",   "-h 13 -b %d") );
	g_rates.insert( pair<string, string>("MCS14",   "-h 14 -b %d") );
	g_rates.insert( pair<string, string>("MCS15",   "-h 15 -b %d") );
	g_rates.insert( pair<string, string>("MCS16",   "-h 16 -b %d") );
	g_rates.insert( pair<string, string>("MCS17",   "-h 17 -b %d") );
	g_rates.insert( pair<string, string>("MCS18",   "-h 18 -b %d") );
	g_rates.insert( pair<string, string>("MCS19",   "-h 19 -b %d") );
	g_rates.insert( pair<string, string>("MCS20",   "-h 20 -b %d") );
	g_rates.insert( pair<string, string>("MCS20",   "-h 21 -b %d") );
	g_rates.insert( pair<string, string>("MCS22",   "-h 22 -b %d") );
	g_rates.insert( pair<string, string>("MCS23",   "-h 23 -b %d") );
...
int LP_remove_dut(void)
{
	...
	try 
	{
		...
		GetPrivateProfileString("Configuration","Disable_WiFi1","0",    szDisablewf1,sizeof(szDisablewf1),iniPath);
...
int LP_tx_set_antenna(void)
{
	...
	try
	{
		...
		*/
		...
		wl -i eth1 ap 1
		wl -i eth1 txchain 0x1
		wl -i eth1 phy_watchdog 0
		wl -i eth1 vht_features 3
		wl -i eth1 rateset 6b
		wl -i eth1 band b
		wl -i eth1 spect 0
		wl -i eth1 chanspec 1
		wl -i eth1 txbf_bfe_cap 1
		wl -i eth1 mpc 0
		wl -i eth1 txbf 0
		wl -i eth1 up
		wl -i eth1 nrate -r 6
		wl -i eth1 ssid 
		wl -i eth1 spatial_policy 1
		wl -i eth1 phy_forcecal 1
		wl -i eth1 txpwr1 -o -q 80
		wl -i eth1 scansuppress 1		
		*/
		if(g_channel <= 14)
		{
			char szCmd[256]="";
			SendDutCmd("","wl -i eth1 down");
			SendDutCmd("","wl -i eth2 down");
			SendDutCmd("","wl -i eth3 down");
			SendDutCmd("","wl -i eth1 country ALL");
			SendDutCmd("","wl -i eth1 ap 1");
			
			//set DUT antane command
			int nAntNo=-1;
			...
			else if(strstr(g_wifi_data_rate,"OFDM"))	// OFDM, DSSS, support one ant only.
			{
				...
				case 1:
					if(g_currentAnt1)
					{					
						SendDutCmd("","wl -i eth1 txchain 0x1");	
					}
					else if(g_currentAnt2)
					{
						SendDutCmd("","wl -i eth1 txchain 0x2");				
					}
					else if(g_currentAnt3)
					{
						SendDutCmd("","wl -i eth1 txchain 0x4");	
					}
					else
					{
						SendDutCmd("","wl -i eth1 txchain 0x8");	
					}
					break;
				case 2:
					//reseverd.
			...
			SendDutCmd("","wl -i eth1 phy_watchdog 0");
			SendDutCmd("","wl -i eth1 vht_features 3");
			SendDutCmd("","wl -i eth1 rateset 6b");
			SendDutCmd("","wl -i eth1 band b");
			SendDutCmd("","wl -i eth1 spect 0");

			//set DUT channel
			strset(szCmd,'\0');
			sprintf_s(szCmd,"wl -i eth1 chanspec %d", g_channel);
			SendDutCmd("", szCmd);

			SendDutCmd("","wl -i eth1 txbf_bfe_cap 1");
			SendDutCmd("","wl -i eth1 mpc 0");
			SendDutCmd("","wl -i eth1 txbf 0");
			SendDutCmd("","wl -i eth1 radio on");
			SendDutCmd("","wl -i eth1 up");
			SendDutCmd("","wl -i eth1 nrate -r 6");
			SendDutCmd("","wl -i eth1 ssid \"\"");
			SendDutCmd("","wl -i eth1 spatial_policy 1");

			//set DUT target power value
			strset(szCmd,'\0');
			sprintf_s(szCmd,"wl -i eth1 txpwr1 -o -q %.1f", g_wifi_tx_power*4);
			SendDutCmd("", szCmd);
			SendDutCmd("","wl -i eth1 txpwr1 scansuppress 1");		
		}
		...
		*/
		...
		wl -i eth2 down
		wl -i eth3 down
		wl -i eth3 txbf_bfr_cap 1
		wl -i eth3 down
		wl -i eth3 country ALL
		wl -i eth3 ap 1
		wl -i eth3 txchain 0x1
		wl -i eth3 phy_watchdog 0
		wl -i eth3 band a
		wl -i eth3 spect 0
		wl -i eth3 chanspec 165
		wl -i eth3 txbf_bfe_cap 1
		wl -i eth3 mpc 0		wl -i eth3 txbf 0
		wl -i eth3 up
		wl -i eth3 nrate -r 6 
		wl -i eth3 ssid 
		wl -i eth3 spatial_policy 1
		wl -i eth3 phy_forcecal
		wl -i eth3 txpwr1 -o -q 60
		wl -i eth3 scansuppress 1
		wl -i eth3 pkteng_start 00:11:22:33:44:55 tx 20 1500 0
		*/
		else
		{
			char szCmd[256]="";
			...
			SendDutCmd("","wl -i eth2 down");
			SendDutCmd("","wl -i eth3 down");
			SendDutCmd("","wl -i eth3 txbf_bfr_cap 1");
			SendDutCmd("","wl -i eth3 down");
			SendDutCmd("","wl -i eth3 country ALL");
			SendDutCmd("","wl -i eth3 ap 1");
			...
			else if(strstr(g_wifi_data_rate,"OFDM"))	// OFDM, DSSS, support one ant only.
			{
				...
				case 1:
					if(g_currentAnt1)
					{					
						SendDutCmd("","wl -i eth3 txchain 0x1");	
					}
					else if(g_currentAnt2)
					{
						SendDutCmd("","wl -i eth3 txchain 0x2");				
					}
					else if(g_currentAnt3)
					{
						SendDutCmd("","wl -i eth3 txchain 0x4");	
					}
					else
					{
						SendDutCmd("","wl -i eth3 txchain 0x8");	
					}
					break;
				case 2:
					//reseverd.
			...
			SendDutCmd("","wl -i eth3 phy_watchdog 0");
			SendDutCmd("","wl -i eth3 band a");
			SendDutCmd("","wl -i eth3 spect 0");

			//set DUT channel
			strset(szCmd,'\0');
			sprintf_s(szCmd,"wl -i eth3 chanspec %d", g_channel);
			SendDutCmd("", szCmd);

			SendDutCmd("","wl -i eth3 txbf_bfe_cap 1");
			SendDutCmd("","wl -i eth3 mpc 0");
			SendDutCmd("","wl -i eth3 txbf 0");
			SendDutCmd("","wl -i eth3 radio on");
			SendDutCmd("","wl -i eth3 up");
			SendDutCmd("","wl -i eth3 nrate -r 6");
			SendDutCmd("","wl -i eth3 ssid \"\"");
			SendDutCmd("","wl -i eth3 spatial_policy 1");
			SendDutCmd("","wl -i eth3 phy_forcecal");

			//set DUT target power value
			strset(szCmd,'\0');
			sprintf_s(szCmd,"wl -i eth3 txpwr1 -o -q %.1f", g_wifi_tx_power*4);
			SendDutCmd("", szCmd);
			SendDutCmd("","wl -i eth3 txpwr1 scansuppress 1");			
		}		
...
int LP_tx_start(void)
{
	...
	try
	{
		...
		if(g_channel>14)
		{		
			SendDutCmd("","wl -i eth3 pkteng_start 00:11:22:33:44:55 tx 20 1500 0");
		}
		else
		{	
			SendDutCmd("","wl -i eth1 pkteng_start 00:11:22:33:44:55 tx 30 1500 0");
...
int LP_tx_stop(void)
{
	...
	try
	{
		...
		*----------------------------*/
		...
		// Here add my codes
		SendDutCmd("","wl -i eth1 pkteng_stop tx");

		SendDutCmd("","wl -i eth3 pkteng_stop tx");
	}
	catch(char *msg)
    {
        vDUT_AddStringReturn(g_LP_DUT_11ac_id, "ERROR_MESSAGE", msg);
