...
int ParseTestItem(char *pszScriptFile)
{
	...
	while(!is.eof())
	{
		...
		if(strstr(szReadLine,"TX_VERIFY_EVM"))
		{
			...
			sprintf_s(txParam.CH_BANDWIDTH,sizeof(txParam.CH_BANDWIDTH), "CBW-%d",atoi(txParam.CH_BANDWIDTH+2));
			sprintf_s(txParam.BSS_BANDWIDTH,sizeof(txParam.BSS_BANDWIDTH), "BW-%d",atoi(txParam.CH_BANDWIDTH+2));
		...
		else if(strstr(szReadLine,"RX_VERIFY_PER"))
		{
			...
			sprintf_s(rxParam.CH_BANDWIDTH,sizeof(rxParam.CH_BANDWIDTH), "CBW-%d",atoi(rxParam.CH_BANDWIDTH+2));
			sprintf_s(rxParam.BSS_BANDWIDTH,sizeof(rxParam.BSS_BANDWIDTH), "BW-%d",atoi(rxParam.CH_BANDWIDTH+2));
			strncpy_s(szTemp1,sizeof(szTemp1)-1,szTemp1+1,strlen(szTemp1+1));
			strncpy_s(szTemp2,sizeof(szTemp2)-1,szTemp2+1,strlen(szTemp2+1));
			strncpy_s(szTemp3,sizeof(szTemp3)-1,szTemp3+1,strlen(szTemp3+1));
